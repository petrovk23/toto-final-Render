...
static void backtrack(
    ...
    // Pre-compute threshold values for aggressive pruning
    double worst_threshold_avg = (*thread_filled >= l) ? thread_best[l - 1].avg_rank : -1.0;
    double worst_threshold_min = (*thread_filled >= l) ? thread_best[l - 1].min_rank : -1.0;
    int is_avg_mode = (strcmp(m, "avg") == 0);
    ...
    for (int num = start_num; num <= max_number; num++) {
        if ((current_S & (1ULL << (num - 1))) == 0) {
            ...
            // CRITICAL OPTIMIZATION: Improved bounds for avg mode
            double upper_avg, lower_avg;
            if (is_avg_mode) {
                // Best case: all remaining subsets have rank total_draws
                upper_avg = (new_sum_current + remaining_subsets * (double)total_draws) / (double)Cjk;

                // Worst case: all remaining subsets have rank 0
                lower_avg = new_sum_current / (double)Cjk;

                // More aggressive pruning for avg mode
                if (*thread_filled >= l && upper_avg <= worst_threshold_avg * 1.001) {
                    continue;  // Skip this branch - cannot improve avg significantly
                }

                // Additional pruning: if we can't beat current worst even in best case
                if (*thread_filled >= l && upper_avg < worst_threshold_avg) {
                    continue;
                }
            } else {
                upper_avg = (new_sum_current + remaining_subsets * (double)total_draws) / (double)Cjk;
            ...
            // Standard pruning logic with improvements
            int should_continue = 0;
            if (*thread_filled < l) {
                should_continue = 1;
            } else {
                if (is_avg_mode) {
                    // Tighter bounds for avg mode
                    if (upper_avg > worst_threshold_avg ||
                        (fabs(upper_avg - worst_threshold_avg) < 1e-9 && new_min_rank > worst_threshold_min)) {
                        should_continue = 1;
                    }
                } else {
                    if (new_min_rank > worst_threshold_min ||
                        (new_min_rank == worst_threshold_min && upper_avg > worst_threshold_avg)) {
                        should_continue = 1;
                    }
                }
            }
...
AnalysisResultItem* run_analysis_c(
    ...
    *out_len = 0;
    ...
    for (int i = 0; i < draws_count; i++) {
        ...
        // Simple bubble sort for each draw
        for (int a = 0; a < 5; a++) {
            for (int b = a + 1; b < 6; b++) {
                if (temp[a] > temp[b]) {
                    int t = temp[a];
