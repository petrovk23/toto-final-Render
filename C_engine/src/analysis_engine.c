...
static void backtrack(
    ...
    for (int num = start_num; num <= max_number; num++) {
        if ((current_S & (1ULL << (num - 1))) == 0) {
            ...
            double upper_avg;
            if (strcmp(m, "avg") == 0) {
                // Tighter bound for average rank
                double max_possible_sum = new_sum_current + (Cjk - Cs_k) * (double)total_draws;
                upper_avg = max_possible_sum / (double)Cjk;
            } else {
                upper_avg = new_sum_current / (double)Cjk; // Not used for min
            }
            ...
            if (*thread_filled < l) {
                ...
                if (strcmp(m, "avg") == 0) {
                    ...
                    if (new_min_rank > thread_best[l - 1].min_rank ||
                        (new_min_rank == thread_best[l - 1].min_rank && new_sum_current / (double)Cjk > thread_best[l - 1].avg_rank)) {
                        should_continue = 1;
                    }
                }
            }
