...
static void evaluate_combo(const int* combo, int j, int k, int total_draws,
                           const SubsetTable* table, const SubsetTable* rank_table, ComboStats* stats);
...
static AnalysisResultItem* run_standard_analysis(
    ...
    // AVG optimization: pre-calculate all k-subset ranks
    SubsetTable* rank_table = NULL;
    if (strcmp(m, "avg") == 0) {
        rank_table = create_subset_table(HASH_SIZE);
        if (!rank_table) {
            free_subset_table(table);
            return NULL;
        }
        int* subset_combo = (int*)malloc(k * sizeof(int));
        if (!subset_combo) {
            free_subset_table(table);
            free_subset_table(rank_table);
            return NULL;
        }
        ...
        while (1) {
            uint64 pat = numbers_to_pattern(subset_combo, k);
            int last_seen = lookup_subset(table, pat);
            int rank = (last_seen >= 0) ? (use_count - last_seen - 1) : use_count;
            insert_subset(rank_table, pat, rank);

            int pos = k - 1;
            while (pos >= 0 && subset_combo[pos] == max_number - k + pos + 1) pos--;
            if (pos < 0) break;
            subset_combo[pos]++;
            for (int x = pos + 1; x < k; x++) {
                subset_combo[x] = subset_combo[pos] + (x - pos);
            }
        }
        free(subset_combo);
    }
            ...
            if (!thread_best) {
                ...
                for (int first = 1; first <= max_number - j + 1; first++) {
                    if (!error_occurred) {  // Check if any thread had allocation error
                        ...
                        while (1) {
                            ComboStats stats;
                            evaluate_combo(curr_combo, j, k, use_count, table, rank_table, &stats);
...
static AnalysisResultItem* run_chain_analysis(
    ...
    while (1) {
        ...
        while (1) {
            ComboStats stats;
            evaluate_combo(combo_buf, j, k, use_count, table, NULL, &stats);
                           ...
                           const SubsetTable* table, const SubsetTable* rank_table, ComboStats* stats)
{
    ...
    while (1) {
        ...
        double rank;
        if (rank_table) {
            rank = (double)lookup_subset(rank_table, pat);
        } else {
            int last_seen = lookup_subset(table, pat);
            rank = (last_seen >= 0)
